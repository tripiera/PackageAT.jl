...
```
le type compatible sélectionné s’affiche immédiatement à l’écran.
Enfin, toute l’application est ensuite regroupée dans une seule page web grâce à la structure suivante :
```julia (editor=true, logging=false, output=true)

```
Grâce à Bonito.jl, le test MBTI a pu être transformé en une application web interactive, plus intuitive que la version console.
L’utilisateur peut saisir ses informations, répondre aux questions par clics, visualiser son type MBTI et choisir un profil compatible de manière totalement graphique.

## 5 Trouver la star compatible avec l'utilisateur
//à completer ici amine
// rshiny ahmed
